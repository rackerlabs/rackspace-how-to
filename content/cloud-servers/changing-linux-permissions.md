...
Put simply, 'chmod' stands for change mode, and this is the command used to set file and/or directory permissions in Linux. This is the command, utilized by the root user, that you will use to determine what users, groups, and others can access files/directories.
...
```
As illustrated above, the use of options with the chmod command is, well, optional. The second example is what is used most frequently. Without an option present, chmod will simply modify the permissions of the file or directory designated in the command.

## Permissions

In a Linux system, permissions is used to refer to the owner of the file or directory (user), the group that owns the file or directory (group), and anyone else who would access the file or directory (others).

There are two ways to state these permissions:
  - alphanumeric characters (r, w, x)
  - octal numbers (0-7)

Example: You have a file called example.txt, and you want to set the permissions as follows:
...
This option uses alphanumeric characters to denote users who will access the file as well as the permissions you desire to set for these designated users.
  - the letter 'u' = user
  - the letter 'g' = group
  - the letter 'o' = other
  - the letter 'r' = read
...
Writing the previous permission set out in how Linux portrays it looks like this:

```sh
-rwxr-xr--
```
The first three (rwx) are the user’s permissions. The second three (r-x) represent the group’s permissions, and the last three (r--) represent the others’ permissions.
...
How are these numbers picked though? Were they just randomly picked from throwing beans in the air? It turns out that no beans were counted in the making of these numbers. They are actually determined from adding together the numbers 4, 2, 1, and 0.
...
Therefore, in our previous example, the 7 is determined from the combination of read (4), write (2), and execute (1). 4 + 2 + 1 = 7. You may already see where this is going. The 5 is taken from adding read (4), none (0), and execute (1). 4 + 0 + 1 = 5. Lastly, the 4 is a combination of read (4), none (0), and none (0). 4 + 0 + 0 = 4. And you thought you'd never use math again... Pfft!
...
As stated previously, you typically will not need to use options in the 'chmod' command, but in case you do, these are here.

## Extras

In addition to the permissions already discussed, there are three more ‘special’ permissions that the ‘chmod’ command is used to set in Linux.
...
This permission is represented as an ‘s’ in the user’s rwx permission set, replacing the ‘x’, as in this example.
...
As you might remember from earlier, the first three letters in ‘rwsr-xr-x’ represent the user’s permissions. Notice, that instead of an ‘x’, there is an ‘s’ in the user’s three permission slots. Instead of showing ‘x’ for execute, there is an ‘s’ in the execute spot. Why is that?
...
The reason for caution may be becoming clearer. If a regular user was given SUID permissions on other binary programs, they could execute commands as ‘root’ without even having ‘root’ permissions. It allows for an abnormal escalation of privileges that usually require sudo privileges to access.
...
This permission is represented as an ‘s’ in the group’s rwx permission set, replacing the ‘x’, as in this example.
...
Like SUID, SGID permissions only work on binaries. They do not work on scripts. As to what SGID does, you may have already guessed that it allows unprivileged group members to execute a binary as if they were the ‘root’ user.

This permission typically is used on directories where members of the group need to be able to have access within the directory shared by the group. Any files created in this directory have the same group owner no matter which group member created the file.

Sticky Bit

This permission is represented by a ‘t’ in the others’ rwx permission set, replacing the ‘x’, as in this example:
...
It is considered ‘best practice’ to set the Sticky Bit permission on any directory whose ‘others’ permission is octal 7 (read, write, execute). In our ‘754’ example, you would want to use the sticky bit permission if the octals were ‘757’ where 7 is the third octal (representing the ‘others’ permissions).

## File / Directory Permissions Cheat Sheet

You may have been wondering throughout this article what the outputs in our examples are comprised of. Here’s a quick cheat sheet explaining each part.

    > Example: -rwxrw-r--  rack  space  123G  Feb  03  15:36  example.txt
