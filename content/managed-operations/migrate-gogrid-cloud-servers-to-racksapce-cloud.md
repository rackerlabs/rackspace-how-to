...
2. Create a new destination server in the Rackspace Public Cloud and record
   ...
   operating system (OS), such as Ubuntu 12, contact Rackspace Support to help
   you build the destination server.

3. If you are running an EOL OS, you need to change the base repos for your
   server (both your old GoGrid server and the new Rackspace Cloud server). For
   example, CentOS 5 should be repo `baseurl=http://vault.centos.org/5.11/os/$basearch`.
...
#### Rackspace Sevice Level Agreements (SLA)
...
Rackspace Public Cloud has six regions:
...
should be able to migrate to the same size memory server. If you are using all
the disk space on your GoGrid cloud server, choose a larger Rackspace Cloud
